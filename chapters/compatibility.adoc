...
////
*APOLLO-Commented paragraph* Reword this paragraph aligned with our strategy
...
We strongly encourage using compatible API extensions and discourage URI versioning
within OpenAPI Specification to keep URI versioning at deployment level (see <<1002>>
and <<1003>> below). The following guidelines for service providers (<<107>>) enable us
(having Postel’s Law in mind) to make compatible changes without versioning.
...
////
[#1002]
== {MUST} use URI versioning at deployment level *ONLY*
* API versioning {SHOULD} be done using the URI versioning mechanism (e.g. /api/v1/wells).
* API versions {SHOULD} be written in the form vN with N being an integer (e.g. /api/v3/wells).

Two versions should be supported at most at the same time (Native apps need a longer cycle).

[#1003]
== {MUST} not use URI versioning in OpenAPI Specification
