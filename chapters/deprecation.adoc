...
[#186]
== {MUST} collect partner consent on deprecation time span

If the API is consumed by any partner (e.g. APIs with
"company-internal" or larger audience) the API owner must define a reasonable
time span that the API will be maintained after the producer has
announced deprecation. All partners must state consent with this
