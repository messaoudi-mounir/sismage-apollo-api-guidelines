...
[#1006]
== {MUST} reference a data catalog for API with *"company-internal" or larger audience*

The design of API data model MUST consider the application use cases, as well as the upstream
and downstream systems, use cases.
The design of the data model of an API MUST consider either the GIDEA Logical Data Model of the
application (internal link) or a Standard data model in Oil & Gas Industry as data catalog reference.
Once the choice has been made, different names for entities and attributes MAY be used ONLY if
the data model chosen as reference is not including them. 
The consistency of the names of entities and attributes across the whole API of a same product MUST
be maintained. 

The reference data catalog associated with an API is mandatory and {MUST} be provided as a specific
extension of Open API standard and must conform to the following specification:

[source,yaml]
----
x-total-mainProperty:
  type: string
  x-extensible-enum:
    - GIDEA
    - WITSML_20
...
    - WITSML_14
  description: |
    Defined data catalog of the property.
----

[#1007]
== {MUST} try to reconcile terms with Standard data model in Oil & Gas Industry for API with *"company-internal" or larger audience*

When a property *can be mapped* to a widely used Standard in Oil & Gas Industry such as OSDU or
Energistics (as per CR EP INF 012), this mapping {MUST} be provided as a specific property 
managed as an extension of Open API standard.
...
The use of custom media is not recommended anymore and should be avoided.
Instead, just use the standard media type name `application/json` 
(or `application/problem+json` for <<176>>).
...
| `string` | `email` | {RFC-5322}[RFC 5322] | `"example@cig-apollo.fr"`
| `string` | `gtin-13` | {GTIN}[GTIN] | `"5710798389878"`
| `string` | `hostname` | {RFC-1034}[RFC 1034] | `"www.cig-apollo.fr"`
...
| `string` | `uri` | {RFC-3986}[RFC 3986] | `"https://www.cig-apollo.fr/"`
