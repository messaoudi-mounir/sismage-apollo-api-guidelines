...
As a
Subsurface Delivery Center we want to deliver products to our (internal)
customers which can be consumed like a service.

SISMAGE API & Apollo Platform products provide their functionality via (internal) APIs; hence,
...
API First is one of our recommendation detailed in these guidelines. In a nutshell API First requires two
aspects:
...
<<1001, API review process>> as to get early review feedback from
